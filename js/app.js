...
async function decryptWithKey(encBase64, key) {
    ...
    try {
        ...
        if (!dec) {
            console.warn("⚠️ [Decrypt] Wrong Key or Corrupted");
            return "🔒 Ошибка ключа";
        }
...
function getChatById(id) { 
    return store.chats.find(c => c.id.toLowerCase() === id.toLowerCase()); 
}
...
function renderChatList() {
    ...
    contactsStore.list.forEach(c => { 
        if (!deletedChatsStore.has(c.address) && !all.find(ch => ch.id.toLowerCase() === c.address.toLowerCase())) {
            all.push({ 
                id: c.address, 
    ...
    all.forEach(chat => {
        ...
        item.className = 'chat-item' + (store.currentChat && store.currentChat.toLowerCase() === chat.id.toLowerCase() ? ' active' : '');
...
function renderMessages() {
    ...
    chat.messages.forEach(msg => {
        ...
        const encIcon = msg.encrypted ? `<span class="msg-enc" title="E2E Encrypted">🔐</span>` : '';
...
async function loadMessages(chatId, start = 0) {
    ...
    try {
        ...
        renderChatList();
        if (store.currentChat && store.currentChat.toLowerCase() === chatId.toLowerCase()) renderMessages();
...
function deleteChat(id) { 
    ...
    if (store.currentChat && store.currentChat.toLowerCase() === id.toLowerCase()) { 
        store.currentChat = null; 
        renderWelcome(); 
    } 
    renderChatList(); 
...
function openContactsModal() { openModal('contacts-modal'); }
