...
async function getSharedKey(peer) {
    ...
    // Normalize and sort addresses to ensure consistent key generation
    const addresses = [userAddress.toLowerCase(), peer.toLowerCase()].sort();
    const cacheKey = addresses.join(':');

    // Check Cache First (Prevents repeated MetaMask prompts)
...
function renderChatList() {
    ...
    contactsStore.list.forEach(c => { 
        if (!deletedChatsStore.has(c.address) && !all.find(ch => ch.id === c.address)) {
            all.push({ 
                id: c.address, 
    ...
    all = all.filter(c => 
        (store.currentFolder === 'all' || c.folder === store.currentFolder) && 
    ...
    all.forEach(chat => {
        ...
        info.innerHTML = `
            ...
            <div class="chat-row2">
                <span class="chat-preview">${escHtml(chat.preview||'')}</span>
...
function updateBadges() {
    ['all','personal','news','work'].forEach(f => {
        const cnt = store.chats.filter(c => 
            !deletedChatsStore.has(c.id) && 
            (f==='all'||c.folder===f) && 
            c.unread > 0
...
function updateSidebarAvatar() {
    ...
    if (userAddress) {
        ...
        const da = document.getElementById('dropdown-avatar-circle'), 
              dn = document.getElementById('dropdown-username'), 
              dd = document.getElementById('dropdown-address');
...
async function loadMessages(chatId, start = 0) {
    ...
    try {
        ...
        const msgs = await Promise.all(all.map(async m => {
            ...
            // Decrypt received messages using the cached key
            if (!isSent) {
                const key = sharedKeyCache.get([userAddress.toLowerCase(), chatId.toLowerCase()].sort().join(':'));
                if (key) {
        ...
        else chat.messages = [...msgs, ...chat.messages];
    ...
    try {
        ...
        if (chat) {
            ...
            chat.messages.push({ 
                ...
                text, // Show plain text to sender immediately
                sent:true, 
                time:now, 
                signature:sig, 
                sender:userAddress, 
...
async function scanForNewSenders() {
    ...
    try {
        ...
        events.forEach(e => { 
            if (e.args.sender.toLowerCase() !== userAddress.toLowerCase()) {
                newSenders.add(e.args.sender); 
            }
        });
...
function openContactsModal() { 
    ...
    if (listEl) {
        if (!contactsStore.list.length) {
            listEl.innerHTML = '<p style="color:var(--text-muted); font-size:13px;">Нет контактов</p>';
        } else {
            listEl.innerHTML = '';
            contactsStore.list.forEach(c => {
                const item = document.createElement('div');
                ...
                item.innerHTML = `
                    <div class="contact-info">
                        <div class="contact-name">${escHtml(c.username || 'Без имени')}</div>
                        <div class="contact-addr">${c.address.slice(0,10)}...${c.address.slice(-4)}</div>
                    </div>
                    <div class="contact-actions">
                        <button class="contact-btn-chat" onclick="selectChat('${c.address}'); closeModal('contacts-modal');">Чат</button>
                        <button class="contact-btn-del" onclick="deleteChat('${c.address}'); openContactsModal();">Удалить</button>
                    </div>
                `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try { 
        const lb = localStorage.getItem('w3m_lastBlock'); 
        if (lb) lastScannedBlock = parseInt(lb); 
    } catch {}
